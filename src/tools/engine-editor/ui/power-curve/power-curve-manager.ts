...
export default class PowerCurveManager extends EventEmitter {
  private bars: number;
  ...
  constructor(options: PowerCurveOptions = {}) {
    ...
    // Initialize arrays
    this.fullThrottleTorquePoints = Array(this.bars).fill(0.5);
  ...
  public getBarWidth(canvasWidth: number): number {
    return (canvasWidth - 2 * this.margin) / this.bars;
  }
