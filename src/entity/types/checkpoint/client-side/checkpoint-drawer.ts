...
import SpawnzoneComponent from "../server-side/server-checkpoint-component";
import TeamColor from "src/utils/team-color";
import TransformComponent from "src/entity/components/transform/transform-component";
import LineDrawer from "src/client/graphics/drawers/line-drawer";

export default class CheckpointDrawer extends EntityDrawer {
    ...
    draw(phase: DrawPhase) {
        ...
        let teamColor = TeamColor.noTeamColor.getUint32()
        ...
        for(let i = 0; i < CheckpointDrawer.vertices.length; i += 2) {
            let x = CheckpointDrawer.vertices[i]
            let y = CheckpointDrawer.vertices[i + 1]
