...
class Program extends EventEmitter {
  ...
  setupTput() {
    ...
    this.put = function() {
      ...
      if (tput.terminfo.methods[cap]) {
        return this._write(tput.terminfo.methods[cap].apply(tput, args));
      }
    };

    for (const key of Object.keys(tput.terminfo.methods)) {
      if (typeof tput.terminfo.methods[key] !== 'function') {
        self.put[key] = tput.terminfo.methods[key];
        continue;
      }

      self.put[key] = function() {
        return self._write(tput.terminfo.methods[key].apply(tput, arguments));
      }
    }
  }
